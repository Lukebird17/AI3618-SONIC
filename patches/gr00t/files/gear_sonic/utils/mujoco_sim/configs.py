...
def override_wbc_config(
    ...
    key_to_value = {
        ...
        "VIEWER_DT": (
            float(config.viewer_dt)
            if config.viewer_dt is not None
            else wbc_config.get("VIEWER_DT", 0.02)
        ),
        "SIM_REALTIME": config.sim_realtime,
...
class BaseConfig(ArgsConfigTemplate):
    ...
    viewer_dt: float | None = 0.066666
    """MuJoCo viewer refresh period (s). Larger = faster physics on WSL (0.05–0.1 ok)."""

    sim_realtime: bool = False
    """If False, run physics as fast as possible (recommended on WSL)."""
    ...
    def __post_init__(self):
        # WSL MuJoCo: source ~/vr/scripts/wsl_dds_env.sh sets SONIC_DDS_INTERFACE=eth2
        env_iface = os.environ.get("SONIC_DDS_INTERFACE")
        if env_iface:
    ...
    def load_wbc_yaml(self) -> dict:
        ...
        if self.robot_scene:
            wbc_config["ROBOT_SCENE"] = _resolve_robot_scene(self.robot_scene)

        return wbc_config


def _resolve_robot_scene(name_or_path: str) -> str:
    """Map short scene aliases to gear_sonic XML paths."""
    key = name_or_path.strip().lower().replace("-", "_")
    presets = {
        "playground": "gear_sonic/data/robot_model/model_data/g1/scene_quest_playground.xml",
        "quest_playground": "gear_sonic/data/robot_model/model_data/g1/scene_quest_playground.xml",
        "counter": "gear_sonic/data/robot_model/model_data/g1/scene_quest_playground.xml",
        "multitask": "gear_sonic/data/robot_model/model_data/g1/scene_quest_multitask.xml",
        "quest_multitask": "gear_sonic/data/robot_model/model_data/g1/scene_quest_multitask.xml",
        "gr00t_lab": "gear_sonic/data/robot_model/model_data/g1/scene_quest_multitask.xml",
        "gr00t": "gear_sonic/data/robot_model/model_data/g1/scene_quest_multitask.xml",
        "lab": "gear_sonic/data/robot_model/model_data/g1/scene_quest_lab.xml",
        "quest_lab": "gear_sonic/data/robot_model/model_data/g1/scene_quest_lab.xml",
        "empty": "gear_sonic/data/robot_model/model_data/g1/scene_43dof.xml",
        "floor": "gear_sonic/data/robot_model/model_data/g1/scene_43dof.xml",
        "43dof": "gear_sonic/data/robot_model/model_data/g1/scene_43dof.xml",
    }
    if key in presets:
        return presets[key]
    if name_or_path.endswith(".xml"):
        return name_or_path
    return presets.get("playground", name_or_path)
...
class SimLoopConfig(BaseConfig):
    ...
    enable_fpv_stream: bool = False
    """Stream MuJoCo head FPV as MJPEG for Quest Browser"""

    fpv_port: int = 8765
    """HTTP port for MuJoCo FPV MJPEG stream"""

    fpv_hz: float = 20.0
    """FPV stream frame rate"""

    enable_fpv_preview: bool = False
    """Open a native PC window for chest (fpv) camera alongside the third-person viewer."""

    robot_scene: str | None = None
    """Override ROBOT_SCENE XML (short name or path under gear_sonic/data/...)."""
