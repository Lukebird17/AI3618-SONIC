...
namespace SonicQuestMirror.Editor
{
    ...
    public static class QuestMirrorSceneSetup
    {
        ...
        public static void SetupEditorScene()
        {
            ...
            {
                cam.name = "FpvCamera";
                cam.transform.SetParent(viewAnchor.transform, false);
                cam.transform.localPosition = PoseMath.MuJoCoToUnityPosition(0.024f, -0.008f, 0.403f);
            ...
            var questLink = network.AddComponent<QuestLinkRig>();
            network.AddComponent<XrQuestLinkBootstrap>();
            network.AddComponent<EditorDesktopPlayMode>();
        ...
        private static void CreateSpectatorCamera()
        {
            ...
            cam.transform.position = new Vector3(-1.8f, 1.6f, 3.2f);
            cam.transform.rotation = Quaternion.Euler(22f, 145f, 0f);
            cam.fieldOfView = 55f;
            cam.depth = -1;
            ...
            WristErrorArrowView arrows)
        {
            ...
            SetRef(pipeline, "operatorCameraRig", operatorCam);
            ...
            RawImage pipImage)
        {
            ...
            var link = network.GetComponent<QuestLinkRig>();
            var viewAnchor = GameObject.Find("ViewAnchor");
            SetRef(link, "receiver", receiver);
            SetRef(link, "robotRoot", robotRoot);
            if (viewAnchor != null)
                SetRef(link, "viewAnchor", viewAnchor.transform);
            SetRef(link, "fpvCamera", cameraTransform);
            SetRef(link, "operatorCameraRig", operatorCam);
